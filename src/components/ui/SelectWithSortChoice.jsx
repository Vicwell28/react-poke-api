...
export const SelectWithSortChoice = ({
  options = [],
  ...
  const [showSortChoice, setShowSortChoice] = useState(null);

  // Estado para las selecciones con su orden preferido
  ...
  const filteredOptions = useMemo(() => {
    ...
    const filtered = options.filter((option) => {
      const searchIn = `${option.label} ${option.searchText || ""} ${
        option.description || ""
      }`.toLowerCase();
  ...
  // Función para seleccionar una opción con orden específico
  const handleSelectWithOrder = useCallback(
    (option, sortOrder = defaultSortOrder) => {
      const optionWithSort = {
        ...option,
        sortOrder: sortOrder, // 'asc' o 'desc'
      };
      ...
      if (multiple) {
        ...
        // Verificar si ya está seleccionada (comparar solo por value)
        ...
        if (existingIndex !== -1) {
          ...
          newSelected[existingIndex] = optionWithSort;
        } else {
          // Si no existe, agregar nueva
          if (maxSelections && currentSelected.length >= maxSelections) {
            return;
          }
          newSelected = [...currentSelected, optionWithSort];
        }

        updateSelection(newSelected);
      } else {
        updateSelection(optionWithSort);
        setIsOpen(false);
        setSearchTerm("");
        setHighlightedIndex(-1);
      }
  ...
  // Función para manejar clic en opción (mostrar elección de orden)
  const handleOptionClick = useCallback(
    (option) => {
      if (allowSortChoice) {
        setShowSortChoice(option.value);
      } else {
        handleSelectWithOrder(option, defaultSortOrder);
      }
    },
    [allowSortChoice, handleSelectWithOrder, defaultSortOrder]
  );
  ...
  const getSelectedOption = useCallback(
    (option) => {
      ...
      return selectedOptions?.value === option.value ? selectedOptions : null;
    },
    [multiple, selectedOptions]
  );
  ...
  return (
    <div ref={selectRef} className={`relative w-full ${className}`}>
          ...
          {multiple && Array.isArray(selectedOptions) ? (
            ...
            <span
              className={`flex items-center gap-1 ${
                selectedOptions ? "text-gray-900" : "text-gray-400"
              }`}
            >
              {selectedOptions && (
                <>
                  {selectedOptions.sortOrder === "asc" ? (
                    <ArrowUp size={14} />
                  ) : (
                    <ArrowDown size={14} />
                  )}
      ...
      {/* Dropdown */}
      {isOpen && !disabled && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border-2 border-gray-200 rounded-xl shadow-lg z-[9999] animate-in fade-in-0 slide-in-from-top-1 duration-200">
          {/* Search Input */}
          {searchable && (
          ...
          >
            {loading ? (
              ...
              filteredOptions.map((option, index) => {
                ...
                const isShowingSort = showSortChoice === option.value;
                  ...
                  <div key={option.value}>
                    ...
                    <div
                      ...
                      className={`
                        ...
                        ${
                          hasMaxSelections && !isCurrentlySelected
                    ...
                    {isShowingSort && allowSortChoice && (
                      <div className="bg-gray-50 border-t border-gray-200">
                        <div className="px-4 py-2">
                          <p className="text-xs text-gray-600 mb-2 font-medium">
                            Elige el orden para "{option.label}":
                          </p>
                          <div className="flex gap-2">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleSelectWithOrder(option, "asc");
                              }}
                              className="flex items-center gap-1 px-3 py-1 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm"
                            >
                              <ArrowUp size={12} />
                              Ascendente
                            </button>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleSelectWithOrder(option, "desc");
                              }}
                              className="flex items-center gap-1 px-3 py-1 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-sm"
                            >
                              <ArrowDown size={12} />
